...
function uc_syn_user($username, $password,$email,$salt){
	$return = call_user_func(UC_API_FUNC, 'user', 'synuser', array('username'=>$username,'password'=>$password,'email'=>$email,'salt'=>$salt));		
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
