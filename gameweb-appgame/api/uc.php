...
class uc_note {
	...
	function synlogin($get, $post) {
		...
		$query = $_SGLOBAL['db']->query("SELECT uid, username, password FROM ".tname('user')." WHERE uid='$uid'");
		...
		if($member = $_SGLOBAL['db']->fetch_array($query)) {
			...
			$session_arr['uid']=$member['uid'];

			$time=time();
			
			$session_arr['username']=$member['username'];

			$session_arr['password']=md5($member['password'].$time);

			$session_arr['dateline']=$time; 
			
			
			$sql="select * from session where username='".$member['username']."'";

			$userinfo=$_SGLOBAL['db']->fetch_array($_SGLOBAL['db']->query($sql));	

			if($userinfo['uid']){

				$member['password']=$userinfo['password']	;

			}else{

				$member['password']=$session_arr['password'];
			...
			$setarr = array(
				...
				'password' => $password,//本地密码随机生成
				);
			//更新本地用户库
			inserttable('user', $setarr, 0, true);
			$session_arr['uid']=$uid;
			$session_arr['username']=$username;
			$session_arr['dateline']=time();
			$session_arr['password']=md5($password.$session_arr['dateline']);
			inserttable("session",$session_arr);
			ssetcookie('auth', authcode("$session_arr[password]\t$uid", 'ENCODE'), $cookietime);					
		}
