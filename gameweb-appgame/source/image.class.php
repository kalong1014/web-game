...
class image
{
 ...
 function watermark_gd($preview = 0)
 {
  if($this->watermarkstatus && function_exists('imagecopy') && function_exists('imagealphablending') && function_exists('imagecopymerge'))
  {
   ...
   {
    $watermark_file = $this->watermarktype == 1 ? S_ROOT.'/image/mark.png' : S_ROOT.'/image/mark.jpg';//这个地方是水印的url(修改成自己的)
	$watermarkinfo = @getimagesize($watermark_file);
    $watermark_logo = $this->watermarktype == 1 ? @imagecreatefrompng($watermark_file) : @imagecreatefromjpeg($watermark_file);
	if(!$watermark_logo)
    {
     return ;
    }
   ...
   {
 	//echo $this->watermarktext['size']."||".$this->watermarktext['angle']."||".$this->watermarktext['fontpath']."||".$this->watermarktext['text'];die();
