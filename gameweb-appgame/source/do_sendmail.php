...
$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('user')." WHERE sendtime<='$_SGLOBAL[timestamp]' ORDER BY sendtime LIMIT 0,$pernum");
...
//邮件内容
$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('user')." WHERE cid IN (".simplode($cids).")");
while ($value = $_SGLOBAL['db']->fetch_array($query)) {
...
//删除邮件
$_SGLOBAL['db']->query("DELETE FROM ".tname('user')." WHERE cid IN (".simplode($cids).")");
