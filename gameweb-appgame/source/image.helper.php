...
if ( ! function_exists('WaterImg'))
{
    function WaterImg($srcFile, $fromGo='up')
    {
        ...
        include(S_ROOT.'/data/data_watermark.php');
		
        require_once(S_ROOT.'/source/image.class.php');
        ...
        if($fromGo=='down' && $photo_markdown=='0')
        {
         ...
         $TRUEMarkimg = S_ROOT.'/image/'.$photo_markimg;
        ...
        if($photo_marktype == '2')
        {
            if(file_exists(S_ROOT.'/data/simhei.ttf'))
            {
                $cfg_watermarktext['fontpath'] =  S_ROOT.'/data/simhei.ttf';
            }
            else
            {
                return ;
